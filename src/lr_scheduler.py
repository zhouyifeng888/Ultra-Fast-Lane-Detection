...
def warmup_cosine_annealing_lr_V2(lr, steps_per_epoch, warmup_epochs, warmup_init_lr, max_epoch, T_max, eta_min=0., cooldown_epochs=0):
    ...
    for i in range(total_steps):
        ...
        lr_each_step.append(lr)
    for _ in range(cooldown_epochs * steps_per_epoch):
        lr_each_step.append(eta_min)
    return np.array(lr_each_step).astype(np.float32)
