...
def _calculate_correct_fan(tensor, mode):
    ...
    if mode not in valid_modes:
        raise ValueError(
            "Unsupported mode {}, please use one of {}".format(mode, valid_modes))
...
def _fc(in_channel, out_channel, use_se=False):
    if use_se:
        ...
        weight = Tensor(np.reshape(
            weight, (out_channel, in_channel)), dtype=mstype.float32)
    else:
...
class ResidualBlock(nn.Cell):
    ...
    def __init__(self,
        ...
        if self.use_se and self.stride != 1:
            ...
            self.conv2 = _conv3x3(
                channel, channel, stride=stride, use_se=self.use_se)
...
class ResidualBlockBase(nn.Cell):
    ...
    def __init__(self,
        ...
        self.conv1 = _conv3x3(in_channel, out_channel,
                              stride=stride, res_base=self.res_base)
        self.bn1d = _bn(out_channel)
        self.conv2 = _conv3x3(out_channel, out_channel,
                              stride=1, res_base=self.res_base)
    ...
    def _make_layer(self, block, layer_num, in_channel, out_channel, stride, use_se=False, se_block=False):
        ...
        if se_block:
            ...
            resnet_block = block(out_channel, out_channel,
                                 stride=1, use_se=use_se, se_block=se_block)
            layers.append(resnet_block)
        else:
