...
class ParsingNet(nn.Cell):
    def __init__(self, backbone, cls_dim=(37, 10, 4), use_aux=False):
        ...
        if self.use_aux:
            self.aux_header2 = nn.SequentialCell(
                ConvBNRelu(128, 128, kernel_size=3, stride=1, padding=1) if backbone in [
            ...
            self.aux_header3 = torch.nn.Sequential(
                ConvBNRelu(256, 128, kernel_size=3, stride=1, padding=1) if backbone in [
                    '34', '18'] else ConvBNRelu(1024, 128, kernel_size=3, stride=1, padding=1),
                ConvBNRelu(128, 128, 3, padding=1),
                ConvBNRelu(128, 128, 3, padding=1),
            )
            self.aux_header4 = torch.nn.Sequential(
                ConvBNRelu(512, 128, kernel_size=3, stride=1, padding=1) if backbone in [
                    '34', '18'] else conv_bn_relu(2048, 128, kernel_size=3, stride=1, padding=1),
                ConvBNRelu(128, 128, 3, padding=1),
            )
        ...
        self.pool = torch.nn.Conv2d(512, 8, 1) if backbone in [
            '34', '18'] else torch.nn.Conv2d(2048, 8, 1)
        # 1/32,2048 channel
        # 288,800 -> 9,40,2048
        # (w+1) * sample_rows * 4
        # 37 * 10 * 4
